...
class JobService:
    ...
    def update_status(self, job_id: str, status: str, error_detail: str | None) -> GenerationJob:
        job = self._jobs.get_by_id(job_id)
        if job is None:
            raise NotFoundError(f"Job '{job_id}' not found")
        if status not in _VALID_STATUSES:
            raise ValidationError(f"Unknown job status: '{status}'")
        return self._jobs.update_status(job, JobStatus(status), error_detail=error_detail)
