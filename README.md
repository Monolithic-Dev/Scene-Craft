...
**Phase 2 (Script/Shot Breakdown Agent) is complete.** A script upload now produces a real, structured scene/shot breakdown, generated by Gemini and verified end to end (see `docs/Phases/PHASE-02-BREAKDOWN-AGENT.md`).
...
agents/           Agent orchestrator + per-agent implementations (breakdown_agent live; frame/app_build/critic/iteration are Phase 3+)
...
cd agents      && ruff check . && mypy --strict orchestrator breakdown_agent shared && pytest -v
